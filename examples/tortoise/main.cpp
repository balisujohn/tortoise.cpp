...
bool autoregressive_model_load(const std::string & fname, autoregressive_model & model)
{
    ...
    //buffer_size += 80 * 1024 *ggml_type_sizef(GGML_TYPE_F32); // init conv1d weights
    //buffer_size += 1024 *ggml_type_sizef(GGML_TYPE_F32); // init conv1d bias
...
struct ggml_cgraph * autoregressive_graph(
    ...
    static size_t buf_size = ggml_tensor_overhead()*2 + ggml_graph_overhead();
    ...
    struct ggml_tensor * output = ggml_get_rows(ctx0, model.text_embedding_weights,input);
