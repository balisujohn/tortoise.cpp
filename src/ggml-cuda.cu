...
static void ggml_cpy_f32_f32_cuda(
    ...
    const int ne00, const int ne01, const int ne02, const int nb00, const int nb01, const int nb02,
    const int nb03, const int ne10, const int ne11, const int ne12, const int nb10, const int nb11, const int nb12,
    const int nb13, cudaStream_t stream) {
...
static cudaError_t ggml_cuda_cpy_tensor_2d(
    ...
    if (nb0 == ts && nb1 == ts*ne0/bs) {
        cudaError_t r = cudaMemcpyAsync(dst_ptr, x, i1_diff*nb1, kind, stream);
        return r;
    } else if (nb0 == ts) {
        cudaError_t r =  cudaMemcpy2DAsync(dst_ptr, ts*ne0/bs, x, nb1, ts*ne0/bs, i1_diff, kind, stream);
        return r;
    } else {
